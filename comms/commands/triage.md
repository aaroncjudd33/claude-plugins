...
Check `to_accept` first (before sender-only rules). For each match, find the calendar event using the dedicated `list_events` tool (pass a start/end date window, then match by subject client-side) — **not** a `search_actions` "calendarView" query, which the capability is not registered under and will falsely read as missing — then call `calendar.respond_to_event` with `response=accept, sendResponse=false` (or `calendar.delete_event` for remove), then **mark the email as read** via `mail.mark_read`, then move it to deleted items via `mail.move` with `folder=deleteditems`. Note: there is no `mail.delete` action.
