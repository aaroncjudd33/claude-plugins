...
# /teams:send [chat-name]

Send a formatted Teams message based on the current conversation context.

## Steps

1. **Identify the target chat** from the argument (e.g. `notes-migration`). Look up the chat ID in `skills/teams/references/known-chats.md`. If the name is not found, ask the user which chat to send to.
...
3. **Compose the message** following all rules in the Teams Messaging Skill:
   - Intro paragraph
   - HTML sections with `<h3>` headers and `<br>` after each
   - Bulleted lists, tables as appropriate
