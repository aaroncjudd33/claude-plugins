...
**Working-tree absence is not the same as "no project memory" (acp-ajudd#137e).** `<repo_root>/.claude/memory/` can be committed on one branch and absent from another — a feature branch cut from `master` before a migration commit reached it will show `local-fallback` even though rich project memory exists elsewhere in the same repo. Before concluding memory doesn't exist, check history: `git log --all -- .claude/memory/MEMORY.md`. If found only on another branch, treat memory as present — read files via `git show <branch>:<path>` (no checkout needed) instead of a local file read, and note which branch it's coming from.
