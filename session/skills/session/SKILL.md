...
- **Consumed = frozen — no post-conversion writes, changes spawn new records (acp-ajudd#59).** The corollary of consumption: once converted, the item takes **no further writes from any role** — not refine, not dispatch, not the coding session itself. Anything the build turns out to need becomes a **new inbox item / story**, never a reopen of the consumed one (implementation moves fast; nothing waits on the retired item). This is a hard line, and it is the inbox half of the per-role write-authority set in § The three roles § Inbox write-authority.
...
1. **Consumed = frozen (hard line).** Once `code #X` converts a record (fold-then-archive — § State-Exclusivity), the item is **frozen: no writes to it after conversion, by any role.** It is gone from the live inbox, and the archived `[CONSUMED … → session <name>]` copy is history, not an editable record. A change the build turns out to need becomes a **new inbox item / story** — never a reopen of the consumed one. Rationale: implementation moves fast and nothing waits on the retired item. (Hardens acp-ajudd#13/#40.)
2. **`refine` = the inbox author, writing surgically.** The one writer of records (§ One inbox writer): it creates / edits / archives items with **targeted, single-item edits**. It **never** does a bulk cross-item, whole-file rewrite of `_inbox.md` — that fragility is what misfiled an item during the #57 pilot (the visible incident was a refinement-side script, not a coding or dispatch violation).
3. **`dispatch` = read-only + notes-only.** It reads and bundles `ready` items and feeds handoff notes to `code`; it **never** creates, edits, archives, or consumes an inbox item, and **never** analyzes requirements or audits code for correctness (that is `code`'s job — dispatch only validates the returned tree against the item's Done-whens). Being purely a reader/coordinator is exactly why a second sessionless context is legal without racing the writer (§ One inbox writer).
4. **`code` = unrestricted (pre-conversion).** It may edit a *live* item freely before conversion (the plan-then-code-in-one-session path — § State-Exclusivity); it consumes/archives the item at conversion, after which rule 1 freezes it. No lockdown.

**Not a concurrency lock.** With dispatch read-only, `refine` and `code` remain the only inbox writers, and the sequential handoff flow (one session active on a given item at a time) plus surgical single-item edits are what keep writes safe — there is no mutex, no hook, no policing (consistent with acp-ajudd#1). Like the rest of the dispatch model, this is **inbox-zone only** (plugin / personal); work repos stay the plain Jira flow with no dispatch layer.
...
5. **If the post-hoc look finds something off** → dispatch hands back a `FIX` note → code fixes and runs **one more deployment.** Both checks still happen; they are sequential and non-blocking. The fix rides in the **coding session** (a re-deploy), **not** by reopening the consumed inbox item — the item is frozen at conversion, and anything genuinely new becomes a fresh record (§ The three roles § Inbox write-authority, acp-ajudd#59).
