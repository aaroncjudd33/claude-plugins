...
- **`[DONE] <ids> — deployed v<x.y.z>. Carry the block below to <role>, then close this terminal; open a fresh one for the next handoff.`** — build finished, terminal closeable (the #80 happy path; adapt the deploy clause to what actually shipped).
...
- **`[HOLD] <ids> — <what to look at>`** — dispatch's hold signal.
- **The dispatch close-signal maps IN, it is not a fifth shape:** `SAFE-TO-CLOSE` is voiced as the `[DONE]` line (e.g. `[DONE] <ids> SAFE-TO-CLOSE — validated; the coding terminal can be closed.`); `HOLD` is voiced as the `[HOLD]` line. The bare tokens still fill the footer recap's close-signal slot inside the block.
...
**Two distinct close cues exist on purpose (acp-ajudd#80)** — they fire at different points in the human relay: the **coding-side** `[DONE]` / `[!] STOPPED` header (printed by `/session:finish` above its return block) tells the courier what to do with the *coding terminal* the moment the build ends; the **dispatch-side** `SAFE-TO-CLOSE` / `HOLD` fires *later*, after dispatch validates the tree. Both are needed; neither replaces the other.
...
4. **Dispatch confirms post-hoc.** On the return note, dispatch validates the **actual working tree** against the Done-whens — reading the diff / the files code produced — **NOT by rubber-stamping the report.** This is **NOT a gate**: the deploy already happened. Dispatch then shows the human a `SAFE-TO-CLOSE` / `HOLD` close-signal, voiced as a courier line (§ Cross-Session Paste Handoff → The courier line).
