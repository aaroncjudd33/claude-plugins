...
**Orchestration is DETECTED, not declared — the keystone (acp-ajudd#74, Pillar 4).** A coding session knows it is a coding session because it **has a session file** (the file dictates the mode); it has **no** knowledge of a dispatch/planning layer above it. It learns it is **orchestrated ONLY by receiving a handoff note.** That single fact settles the whole return-handoff question:
...
- **A handoff note WAS received → orchestrated** → on `/session:finish`, **ALWAYS emit a return handoff** to the sender (dispatch). This is what makes acp-ajudd#72 facet-1 precise: "coding-finish always hands a note back" means *always, IF fed by a handoff note.* `commands/finish.md` owns the mechanics; the detection cue is that the session was created from a dispatch/planning work order (recorded in its "Picked up from inbox" provenance).
...
3. **Happy path — code self-verifies against the Done-whens and FINALIZES by default.** `/session:finish` runs the deploy (bump + push + reinstall — § Development Lifecycle). **No HOLD.** Because the session was **orchestrated** — it detected this by having received the leg-1 handoff note (acp-ajudd#74, Pillar 4 — orchestration is detected, not declared) — finish **always** returns a `State: IMPLEMENTED-DEPLOYED` handoff block to dispatch (command-invoked so it comes back *as a block*, acp-ajudd#43), then it is free to pick up the next item or close out. A **solo** session (no note ever received) skips the return — there is no one to report to.
