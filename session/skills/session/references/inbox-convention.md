...
**The `scope:` field (acp-ajudd#161, lite pickup only).** A lite pickup (§ Pickup state below) writes a third field onto the same line: `> [type: work · status: in-progress · scope: <set>]` — the lite-work equivalent of a session file's `Scope:` field, derived at pickup the same way (`references/lite-mode.md`). Parsed independently, same tolerant rule as `type`/`status`: absent → the Scope Scan step in `checkpoint`/`commit`/`finish` simply skips (never errors), same as a session file with no `Scope:` field. Only ever appears on an `in-progress` lite item — a session-graduated pickup never carries it (its scope lives on the session file instead).
...
Pickup state is **orthogonal to the maturity `status` above**: `status` = "is this work scoped enough to grab" (`new` → `refining` → `ready`); pickup state = "has a session grabbed it yet." A `new`/`refining`/`ready` work entry is "pending" here; `code`-ing it makes it in-progress (`new`/`refining` just means `code` warns first). **One exception:** a *lite* pickup (item-driven inbox only, below) carries pickup state ON the `status` line itself (`status: in-progress`) rather than a separate marker, since the entry never leaves the inbox to fold into anything — there is no second surface to put it on.

**Two pickup mechanisms, by inbox kind — plus a third, lite variant of the first:**
- **Item-driven consolidated inbox (plugin / personal — `_inbox/`), session pickup:** pickup **consumes** the entry — **fold-then-archive** (state-exclusivity, above): the body folds into the session file, a `[CONSUMED <date> → session <name>]` copy is appended to `_inbox_archive.md`, and the item's `_inbox/<id>.md` file is deleted. The session (with the archived copy as a backstop) is the paper trail.
- **Item-driven consolidated inbox, *lite* pickup (acp-ajudd#154/#143 — `references/lite-mode.md`):** an explicit opt-out of the session file at pickup time (default is still session pickup, above). The entry is **not** consumed — its metadata line flips to `> [type: work · status: in-progress · scope: <set>]` in place (the `scope:` field per acp-ajudd#161, above) and it **stays live** in `_inbox/<id>.md`, functioning the same role the per-session in-progress marker plays below, just carried on the status line instead of an inserted marker line (the item-driven format has no separate marker line). Updates land as a progress-log append to the same file. Closes exactly like the per-session flow: stays until done, then archived with a `[DONE]` stamp — never consumed-on-pickup, because there was never a session to fold into.
