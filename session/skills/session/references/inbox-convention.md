...
Pickup state is **orthogonal to the maturity `status` above**: `status` = "is this work scoped enough to grab" (`new` → `refining` → `ready`); pickup state = "has a session grabbed it yet." A `new`/`refining`/`ready` work entry is "pending" here; `code`-ing it makes it in-progress (`new`/`refining` just means `code` warns first). **One exception:** a *sessionless* pickup (item-driven inbox only, above) carries pickup state ON the `status` line itself (`status: in-progress`) rather than a separate marker, since the entry never leaves the inbox to fold into anything — there is no second surface to put it on.

**Two pickup mechanisms, by inbox kind — plus a third, sessionless variant of the first:**
