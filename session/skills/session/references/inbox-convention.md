...
This single file is THE inbox for the slug. `/session:start` reads it and the plugin/personal flow `code`s `work` from it (the old `pick` — acp-ajudd#56) — at graduation, the body is folded into the new feature session and archived-on-consume (a `[CONSUMED …]` copy is appended to `_inbox_archive.md`, then the entry is removed from the live inbox — acp-ajudd#40). For the plugin marketplace, slug = `ajudd-claude-plugins`; for a personal project, slug = that project's directory name. Plugin and personal behave identically.
...
**Legacy (historical, back-compat only — do not write new entries here):**

Before the item-driven-sessions overhaul, multi-plugin repos used one inbox file *per plugin* — `_inbox_session.md`, `_inbox_release.md`, `_inbox_comms.md`, etc. These files (and their `_archive`/`_backlog` siblings) are left in place as history. The listing renderer still reads them for legacy sessions, but the new flow never writes to them. New entries always go to the consolidated `_inbox.md` above.
...
|---|---|---|
| **Plugin marketplace** | `work` in `_inbox.md` (→ file/git history) | the `work` entry |
| **Personal** (`personalProjectsDir`) | `work` in `_inbox.md` (→ file/git history) | the `work` entry |
...
- **Item-driven consolidated inbox (plugin / personal — `_inbox.md`):** pickup **consumes** the entry — **fold-then-archive** (state-exclusivity, above): the body folds into the session file, a `[CONSUMED <date> → session <name>]` copy is appended to `_inbox_archive.md`, and the entry is removed from the *live* inbox. The session (with the archived copy as a backstop) is the paper trail.
