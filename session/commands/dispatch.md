...
- **Default instruction (both toggles yes):** tell the coding session to **read the entry, build it, self-validate against the Done-whens, and self-finalize (`/session:finish`) if clean — coming back only on the escape hatch** (question / unclear / disagreement / found-problem). No `IMPLEMENTED-DEPLOYED` report, no waiting on you. The footer's return instruction is just the escape-hatch reminder (the happy path returns nothing).
- **Toggle-off instruction (`Self-finalize: no`):** tell the coding session to self-verify, **deploy on its own authority (NO HOLD)**, then **report `State: IMPLEMENTED-DEPLOYED` and STOP — do NOT run `/session:finish`; shipping is not closing (acp-ajudd#94).** The session stays open awaiting your Step-6 validation and `Action: FINISH` close-order. Here the footer must be **command-invoking** — run `/session:handoff` to reply with the `IMPLEMENTED-DEPLOYED` (or stop-reason) block back to dispatch (acp-ajudd#43).
...
**Wait for the FINISH to be confirmed before releasing the next work order — `Self-finalize: no` path only (acp-ajudd#98/#100, scoped by #109).** In the toggle-off path, do not emit the next `PICK UP` work order (or otherwise treat this entry as done and pull its dependents) until the coding session returns its **`State: FINISHED`** confirmation that `/session:finish` ran — a bare `IMPLEMENTED-DEPLOYED` return is a **ship, not a finish** (shipping ≠ closing). A `[DONE]` archive stamp on the entry is the reliable done-signal. (In the **default** self-finalize path there is no report at all; the `[DONE]` stamp the coding session's own finish writes is the done-signal, and dispatch sequences off the inbox / `depends-on` markers directly.)
