...
**One echoed panel (acp-ajudd#132/#142, inbox collapsed acp-ajudd#159).** The whole plugin-zone panel — `Quick start` (verbs, including the secondary `dispatch`/`capture` lines), `Recent Sessions` (sessions), and `Advanced` (`inbox`/`memory`/`search` — inbox collapses to a count line here, same shape as work/general; this reverses #135/#142's deliberate "inbox primary" call now that start-time speed outweighs an always-visible inbox list) — is rendered by **one script**, the same `start-panel.py` the work zone uses, zone-branched via `--zone plugin`. This retires the old three-piece assembly (`routing-block.py` + `inbox-render.py`'s `pickup` call + `session-list.py`) — do **not** reassemble it by hand. Run it and display stdout **exactly as printed** — no code fence, no restating:
```bash
ROOT="${CLAUDE_PLUGIN_ROOT:-$HOME/.claude/plugins/marketplaces/<pluginMarketplaceName>/session}"
...
```
The `Advanced` inbox count comes from `start-panel.py`'s own `count_inbox_work()`. The full pickup list is one word away — the `inbox` keyword below (§ Type-specific accepted inputs) reuses `inbox-render.py`'s `render_pickup()`/`collect_work_items()` unchanged — layout B, `[spawn]` starred, `· <stage>` suffix for `new`/`refining`, provenance dimmed, and the trailing "Captures waiting: N" glance all carry over. Reading captures (on request only — `check captures`, or "read the capture from `<repo>`") still follows `references/inbox-convention.md` § Captures inbound: disposition each — **promote** (→ `refining`), **discard**, **absorb into the current session**, or **feed a refinement** — the three non-promote fates **archive** the capture.
...
**Fallback (script unavailable)** — if `python3` is absent, the script exits non-zero, or stdout is empty, render it yourself in the same order and shape: the verbs block (refine/code + dispatch/capture, underlined header), then `Recent Sessions` (`session-list.py --limit 5`, no title column), then `Advanced` (`inbox`/`memory`/`search`, inbox as a count line — `grep`/count `<session_root>/_inbox/*.md` `work`-type entries at `new`/`refining`/`ready` if the script itself is unavailable).
...
- `inbox` (acp-ajudd#159) → render the full pickup list on request: run `inbox-render.py pickup --session-root <session_root> --slug <slug> --current-slug <slug>` and show its stdout verbatim (layout B, `[spawn]` starred, `· <stage>` suffix for `new`/`refining`, provenance dimmed, trailing `Captures waiting: N` glance). Then re-show the panel. **This is the list `code <n>` needs on screen** — see the `<n>` note below.
- `code <n>` / `code <id>` where the target is a **`work` entry** → graduate it into a fresh feature-named coding session, **or pick it up lite** (acp-ajudd#154/#143 — read `start-impl.md`'s Work Pickup step 1a first: it asks session-vs-lite before anything else, default session). On the **session** path: reads start-impl.md, goes to Step 4 (new session): derive a feature name (confirmed once), fold the entry body into the new session, then archive-on-consume — append a `[CONSUMED …]` copy to `_inbox_archive.md`, then **delete the item's `_inbox/<id>.md` file** (fold-then-archive — the session plus the archived copy is the trail; acp-ajudd#40 / #102). On the **lite** path: no session file is written at all — see `references/lite-mode.md`. `<id>` is the stable handle (e.g. `acp-ajudd#3`) — **always valid**, whether or not the list is on screen. `<n>` is the ephemeral inbox list position from the `inbox` keyword's expansion above (acp-ajudd#159 — the panel itself no longer shows this list, so a bare `<n>` only resolves once `inbox` has rendered it this turn; a `<n>` typed with no list on screen is ambiguous against the sessions table instead — see the next bullet). **If the work is not fully scoped — `status: new` or `refining`** (parsed from its `> [type: work · status: …]` line — keyed on status, *not* on origin), warn and confirm first: `[<id>] is not fully scoped (status: <new|refining>) — code it anyway? You'll scope AND build; refine first if it's big. (yes / leave it)`. A `ready` entry (the default) codes with no warning. **Never blocks** — a capable coding session decides based on size. (A `capture`-type entry isn't in this list — `code` a capture only after it's promoted to `work`.)
- `code <n>` / `code <name>` where the target is a **Recent Sessions row** → resume it (the Plugin session resume path in Step 4). `<n>` is the Recent Sessions row (always on screen); `<name>` is the feature name. A bare number that matches a Recent Sessions row **and** a just-expanded inbox row (only possible right after `inbox`) is the only genuinely ambiguous case — ask which; otherwise infer.
...
- `<n>` / `<name>` alone → `code` it (shorthand for the above; same `<n>` scoping rule as above).
...
**Fallback (start-panel.py unavailable)** — if `python3` is absent, the script exits non-zero, or stdout is empty, render the panel yourself in the same order and shape: the `Quick start` verbs (refine `<n|KEY>` / code `<n|KEY>` / release `<KEYS>`, underlined header), then a detailed `In Progress` list from `session-list.py --limit 5` (or `_index.md` if that also fails), then an `Advanced` block (`inbox` / `memory` / `search`). Keep the underlined headers and the folded summary.
...
- `release BPT2-XXXX [...]` — **the primary verb for CAB work (acp-ajudd#158), symmetric with `code`'s "the file decides" behavior:**
  - **One or more story keys** → new CAB coding session for those stories (routes to `/release:create-cab`); if no keys in the reply, ask "Story keys? (space-separated)" as follow-up.
  - **A single `CAB-XXXX` key** → resolves exactly like `code CAB-XXXX` (resume the existing CAB coding session; the file decides).
  - `cab BPT2-XXXX [...]` (bare `cab`, no `code` prefix) and `code cab …` remain accepted as documented **legacy synonyms** for the new-kickoff case — never silently dropped.
  - **Before routing a new kickoff** — session-vs-lite choice (acp-ajudd#154/#143 — `start-impl.md`'s "CAB — new" / `references/lite-mode.md`, default session): asked once, same wording as the story/plugin prompt. `lite` still routes to `/release:create-cab` exactly as today (CAB kickoff mechanics are unaffected); it only changes whether `session` also writes a parallel session-state file on top — updates instead post as Jira comments on the CAB issue. **`release <KEYS> lite`** (a trailing `lite` token, same shortcut as `code <target> lite`; `cab <KEYS> lite` also still works) → pre-answers the question and skips the prompt, announced with one line (e.g. `Going lite on this CAB — no session file, updates via Jira comments on the CAB issue.`).
...
python3 "$ROOT/scripts/start-panel.py" --zone personal --session-root "<session_root>" --slug "<slug>" \
  --handle "<handle>" --current-branch "$BRANCH" --repo-root "$RTOP" --limit 5
```
Same `Quick start` / `Recent Sessions` / `Advanced` (`inbox`/`memory`/`search`, inbox collapsed to a count line — acp-ajudd#159) shape, same echo-compliance rule, same fallback (render it yourself in the same order and shape — see the plugin block above for the full fallback description, identical for personal). Reading captures on request follows the same `references/inbox-convention.md` § Captures inbound flow.

**Type-specific accepted inputs** (plus the shared inputs above):
- `dispatch` → read `commands/dispatch.md` and run it (assume the dispatch role, orient on the inbox — sessionless; creates no session file).
- `capture` → read `commands/capture.md` and run it (assume the capture role, bank ideas as `capture`-type entries — sessionless; creates no session file).
- `inbox` → render the full pickup list on request, same as the plugin block above (`inbox-render.py pickup`), then re-show the panel. **This is the list `code <n>` needs on screen** — see the `<n>` note below.
- `code <n>` / `code <id>` where the target is a **`work` entry** → graduate it into a feature-named coding session (same fold-then-archive flow as plugin: Step 4 → start-impl.md; the entry's `<id>` is preserved in the folded provenance block). `<id>` is always valid; `<n>` is the ephemeral inbox list position and only resolves once `inbox` has rendered it this turn (acp-ajudd#159 — same rule as plugin). Same `new`/`refining` warn/confirm as plugin — keyed on the entry's `status`, not its origin. Never blocks.
- `code <n>` / `code <name>` where the target is a **Recent Sessions row** → resume it. Same disambiguation as plugin (ask only on a genuinely ambiguous bare number matching both a Recent Sessions row and a just-expanded inbox row).
