...
4. Check whether `<session_root>/<name>.md` exists (**this existence check IS "the file decides"** — a session file present means resume-coding; absent means graduate-work or kickoff):
   ...
   - **Does not exist + plugin/personal** → do NOT blank-create. These types are item-driven, but the original `<target>` string must not be silently dropped (acp-ajudd#155 — previously it was discarded here, dead-ending on the bare panel with no attempt to match it). Carry it forward as `unmatched_target` and fall through to Step 1 (full discovery + inbox flow); `start-classic.md`'s Step 3 runs its existing free-text matcher against `unmatched_target` before presenting the bare panel — see the note there.
