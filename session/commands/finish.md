...
- **Orchestrated, default (`Self-finalize: yes`) — the coding session self-finalized (acp-ajudd#109).** The session self-validated and ran this close **itself** — there was no ship-report and there is no close to confirm back. Do **NOT** emit any return block. Go straight to the ✅ / 🏆 close cue below; dispatch sequences off the `[DONE]` stamp this close just wrote (Session Skill § The dispatch↔code loop).
...
```
This is the `✅` **close** cue. In the **default self-finalize path** it is the *single* terminal cue (the session self-validated and self-finalized — no earlier `📤` ship cue fired). In the `Self-finalize: no` path it is distinct from the `📤` **ship** cue printed earlier at build-end and from dispatch's `SAFE-TO-CLOSE` signal. It fires only now, after the atomic close ran (Claude can't restart itself; the cue hands the terminal back to the human to close). When a return block is also emitted (`Self-finalize: no`, or planning-fed solo), print it above the ✅ close cue.
