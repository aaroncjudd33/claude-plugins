...
- **`Action:` / `State:`** — add **`Action:`** on an outbound note (the sender's intent: `PICK UP #X`, `FIX`, `VALIDATE`, `FINISH`, `HALT`) or **`State:`** on a return note (what happened: `IMPLEMENTED-DEPLOYED`, `VALIDATED`, `FINISHED`, `FOUND-ISSUE`, `REQUIREMENTS-CHANGE`, `BLOCKED-QUESTION`, `HALTED`) — one or the other, never both. `FINISH` is the close-order and `IMPLEMENTED-DEPLOYED` / `FINISHED` are its returns — all three exist **only in the `Self-finalize: no` path** (acp-ajudd#109; `FINISH` renamed from `CLOSE`). `HALT`/`HALTED` stand down dispatched work mid-flight (Session Skill § HALT — no publish, no commit, WIP preserved). Omit if this handoff is neither (a bare informational pass).
- **`Self-validate:` / `Self-finalize:` — on a dispatch/planning ──▶ coding work order only (acp-ajudd#109).** Set both fields, default **`yes` / `yes`** (self-validate + self-finalize — the coding session validates its own build and closes itself). Flip one to `no` only when this work wants dispatch's second pair of eyes (`Self-validate: no`) or a deliberate close-order (`Self-finalize: no`). **No tooling** — you write the fields, the human sees them, and a change is a regenerated note. Echo both in the header **and** the footer recap. Omit on any note that is not a coding work order.
- **Close-signal** — on a **dispatch** validation/close note (the `Self-finalize: no` path only), fill the close-signal slot of the footer recap (`SAFE-TO-CLOSE` / `HOLD`) and voice it to the human via the fixed courier-line vocabulary (SKILL § Cross-Session Paste Handoff → The courier line — `SAFE-TO-CLOSE` maps to the `✅` line, `HOLD` to `⏸`). Other roles / the default path omit it.
- **`Re:`** = topic + any inbox IDs / story keys carried (omit the ID clause if none). **`Slug` / `Zone`** = current repo slug + session type. **Footer** = the **one-line state recap** (acp-ajudd#82): `END HANDOFF · [YYYY-MM-DD HH:MM @handle] · from ──▶ to · <ids> · <Action/State> · <self-validate/self-finalize, coding work orders only> · <return instruction> · <close-signal, dispatch legs only>` — one compressed scannable line echoing the header state so the last thing visible in the window orients the human without scrolling up. **When the return leg must come back as a block the return instruction must be command-invoking (acp-ajudd#43, generalized):** tell the receiver to *run `/session:handoff`* to reply with a handoff block — do not have it free-form the report. This applies to a **`Self-finalize: no` dispatch→code work order** (the return `IMPLEMENTED-DEPLOYED` / `FINISHED` / stop-reason must come back as a block); a **default work order's** return instruction is just the escape-hatch reminder (the happy path returns nothing). The SKILL § Cross-Session Paste Handoff owns the exact footer wording; this command does not restate it.
