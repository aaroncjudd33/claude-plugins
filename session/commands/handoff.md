...
**This command drives the moving legs of the dispatch↔code loop.** When work is dispatched, the dispatch→code work order is a `/session:handoff` call — see Session Skill § **The dispatch↔code loop — self-finalize by default (acp-ajudd#109)** for the full round-trip. **Default (`Self-validate: yes` / `Self-finalize: yes`, the work order's default fields):** dispatch hands off with Done-whens → code reads the entry, builds, **self-validates against the Done-whens, and runs `/session:finish` itself** → done. No `implemented-deployed` report, no round-trip wait on the happy path — code **waits for a reply only** on the escape hatch (question / unclear / disagreement / found problem). The close itself DOES emit one **fire-and-forget** `coding ──▶ dispatch` `State: finished` block when the session was dispatch-fed (acp-ajudd#118) — a one-way notify telling dispatch to pull the next item, never a return handoff code waits on. **Toggle-off (`Self-finalize: no`):** code ships by default (no hold), **reports a `State: implemented-deployed` block but does NOT self-close** — shipping is not closing → dispatch validates the tree (if `Self-validate: no`) and **orders the close** (`safe-to-close` / `Action: finish`) → the still-open coding session runs `/session:finish` and returns a `State: finished` confirmation. This command emits every one of those blocks. A solo coding session with no dispatcher doesn't run the loop (it self-finalizes); this command is just its outbound handoff. **Inbox zones only** — work repos have no dispatch role.
...
- **Title border bar** — names **both ends** and the item #, **lowercase** (acp-ajudd#101): `<from-role> ──▶ <to-role> handoff · #<id>` (e.g. `coding ──▶ dispatch handoff · #57`, `dispatch ──▶ coding handoff · #57`, `dispatch ──▶ planning handoff · #57`) — acp-ajudd#69 + #101. `<from-role>` is this context's origin role and always matches the left side of the `Path:` line (acp-ajudd#45); `<to-role>` is the destination and **is the routing instruction** that tells the human which terminal to paste into (load-bearing in the strict-hub topology — § The three roles). The SKILL § Cross-Session Paste Handoff owns the exact title wording; this command does not restate it.
- **`Path:` (`<from-role> (<from-name>) ──▶ <to-role> (<target>)`)** and **`Date:` (`[YYYY-MM-DD HH:MM @handle]`)** — the origin is `coding (<session-name>)` on the coding path, or `dispatch (<slug>)` / `planning (<slug>)` (the refine/refinement role — canonical token `planning`) on the sessionless path (whichever role this context declared); `<target>` is the label from Step 2. **The `HH:MM` comes from a real `date` call run at emit time (e.g. `date +"%Y-%m-%d %H:%M"`) — never guessed** (acp-ajudd#82; the SKILL § owns the mechanism). The same `Date:` stamp appears in the footer (the byte-identical header mirror).
- **`Action:` / `State:`** — add **`Action:`** on an outbound note (the sender's intent: `pick up #X`, `fix`, `validate`, `finish`, `halt`) or **`State:`** on a return note (what happened: `implemented-deployed`, `validated`, `finished`, `found-issue`, `requirements-change`, `blocked-question`, `halted`) — one or the other, never both; **lowercase values** (acp-ajudd#101). `finish` is the close-order and `implemented-deployed` are its return — both exist **only in the self-finalize-off (`Finish: no`) path** (acp-ajudd#109; `finish` renamed from `close`). **`finished` is not toggle-off-only** — it also fires as the one-way, fire-and-forget `coding ──▶ dispatch` return on a **default-path** dispatch-fed close (acp-ajudd#118), in addition to its existing toggle-off finish-confirmation use. `halt`/`halted` stand down dispatched work mid-flight (Session Skill § HALT — no publish, no commit, WIP preserved). Omit if this handoff is neither (a bare informational pass).
- **`Validate:` / `Finish:` — on a dispatch/planning ──▶ coding work order only (acp-ajudd#109).** The block's two toggle lines (`Validate: yes|no` / `Finish: yes|no` — the #109 self-validate / self-finalize toggles), default **`yes` / `yes`** (the coding session validates its own build and closes itself). Flip one to `no` only when this work wants dispatch's second pair of eyes (`Validate: no`) or a deliberate close-order (`Finish: no`). **No tooling** — you write the fields, the human sees them, and a change is a regenerated note. Echo both in the header **and** the byte-identical footer mirror. Omit on any note that is not a coding work order.
- **Close-signal** — on a **dispatch** validation/close note (the self-finalize-off path only), voice the close-signal (`safe-to-close` / `hold`) to the human via the fixed courier-line vocabulary (SKILL § Cross-Session Paste Handoff → The courier line — `safe-to-close` maps to the `✅` line, `hold` to `⏸`). It is **courier-line only** — the footer is a byte-identical header mirror with no close-signal slot (acp-ajudd#101). Other roles / the default path omit it.
- **`Title:`** = `#<id> — <short name>` (replaces the retired `Re:`, acp-ajudd#101; the id leads the field and rides both border bars). **`Slug:`** = `<slug> · <zone>` on one line (no separate `Zone:` label). **Footer** = a **byte-identical mirror of the header** (acp-ajudd#101, retires the #82 one-line recap): the same labeled field-block repeated verbatim, bracketed by the identical border bar (roles + item #), so the last thing visible in the window fully orients the human without scrolling up — **no compressed one-liner**. **When the return leg must come back as a block the return instruction must still be command-invoking (acp-ajudd#43, generalized):** in the self-finalize-off path, the note body / loop protocol tells the receiver to *run `/session:handoff`* to reply with a handoff block — never free-form the report; it is no longer a footer slot. A **default work order** waits on nothing on the happy path (escape hatch only) — the close still emits the one-way, fire-and-forget `finished` note when dispatch-fed (acp-ajudd#118), command-invoked the same way, but dispatch never replies to it. The SKILL § Cross-Session Paste Handoff owns the exact footer wording; this command does not restate it.
